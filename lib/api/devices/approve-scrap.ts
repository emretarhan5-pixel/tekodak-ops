...
const approveScrapSchema = z.object({
  deviceId: z.string().uuid(),
});

export async function approveScrap(
  deviceId: string,
): Promise<ActionResult<{ deviceId: string }>> {
  try {
    const input = approveScrapSchema.parse({ deviceId });
    ...
    const { error: updateError } = await ctx.supabase
      ...
      .update({
        ...
        status: "scrapped",
        scrap_approved_by: ctx.user.id,
        scrap_approved_at: now,
    ...
    if (row.scrapped_by) {
      await notifyUser(row.scrapped_by, {
        type: "scrap_approved",
        title: "Hek Onaylandı ✓",
        message: `${row.serial_number} cihazının hek durumu onaylandı.`,
        entityType: "device",
