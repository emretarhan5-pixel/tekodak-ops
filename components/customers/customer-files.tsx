...
import {
  ...
  CUSTOMER_FILE_CATEGORY_ORDER,
  CUSTOMER_FILE_MAX_BYTES,
  getCustomerFileCategoryLabel,
...
function groupFilesByCategory(files: CustomerFileRow[]) {
  const groups = new Map<string, CustomerFileRow[]>();
  ...
  const ordered: Array<{ category: string; label: string; files: CustomerFileRow[] }> =
    [];

  for (const cat of CUSTOMER_FILE_CATEGORY_ORDER) {
    const list = groups.get(cat);
    if (list?.length) {
      ordered.push({
        category: cat,
        label: getCustomerFileCategoryLabel(cat),
        files: list,
      });
      groups.delete(cat);
    }
  ...
  for (const [category, list] of groups.entries()) {
    if (list.length > 0) {
      ordered.push({
        category,
        label: getCustomerFileCategoryLabel(category),
        files: list,
      });
    }
  }
...
export function CustomerFiles({
  ...
  const [category, setCategory] = useState<CustomerFileCategory>("contract");
  ...
  function canDeleteFile(file: CustomerFileRow): boolean {
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Müşteriye ait belgeler (en fazla 10 MB)
            </CardDescription>
          </div>
          <Button
            type="button"
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border py-14 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Sözleşme, ihale veya fatura belgelerini buradan yükleyin.
                </p>
              </div>
              <Button
                type="button"
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Field>
              <FieldLabel htmlFor="file-category">Kategori</FieldLabel>
              <select
                id="file-category"
                className={selectClassName}
                value={category}
                onChange={(e) =>
                  setCategory(e.target.value as CustomerFileCategory)
                }
                disabled={isUploading}
              >
                {CUSTOMER_FILE_CATEGORIES.map((item) => (
