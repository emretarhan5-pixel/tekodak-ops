...
function groupFilesByCategory(files: DeviceFileRow[]) {
  const groups = new Map<string, DeviceFileRow[]>();
  ...
  for (const cat of DEVICE_FILE_CATEGORY_ORDER) {
    const list = groups.get(cat);
    if (list?.length) {
      ordered.push({
        category: cat,
        label: getDeviceFileCategoryLabel(cat),
        files: list,
      });
      groups.delete(cat);
    }
  ...
  for (const [category, list] of groups.entries()) {
    if (list.length > 0) {
      ordered.push({
        category,
        label: getDeviceFileCategoryLabel(category),
        files: list,
      });
    }
  }
...
export function DeviceFiles({
  ...
  const [category, setCategory] = useState<DeviceFileCategory>("invoice");
  ...
  function canDeleteFile(file: DeviceFileRow): boolean {
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Cihaza ait belgeler (en fazla 10 MB)
            </CardDescription>
          </div>
          {canEdit ? (
            <Button
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border py-14 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Fatura, garanti belgesi veya teknik doküman yükleyin.
                </p>
              </div>
              {canEdit ? (
                <Button
        ...
        >
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              ...
              <Field>
                <FieldLabel htmlFor="device-file-category">Kategori</FieldLabel>
                <select
                  id="device-file-category"
                  className={selectClassName}
                  value={category}
                  onChange={(e) =>
                    setCategory(e.target.value as DeviceFileCategory)
                  }
                  disabled={isUploading}
                >
                  {DEVICE_FILE_CATEGORIES.map((item) => (
              ...
              <Field>
                <FieldLabel htmlFor="device-file-description">
                  Açıklama (opsiyonel)
                </FieldLabel>
                <textarea
                  id="device-file-description"
